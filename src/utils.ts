...
export function loadEmployee(employeeDiscord: string): Employee {
  ...
  const employee = employees.find(
    (employee) => employee.discord === employeeDiscord
  );
  if (employee) {
    return employee;
  } else {
...
export function removeEmployee(discord: string) {
  ...
  const employeesToKeep = employees.filter(
    (employee: Employee) => employee.discord !== discord
  );
  employees.length === employeesToKeep.length
...
export function addEmployee(employee: string, discord: string, mail: string) {
  const employees = loadEmployees();
  const ducplicateEmployee = employees.find(
    (employee: Employee) => employee.discord === discord
  );
